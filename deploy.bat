...
rem ============================================================
rem  sidenote-pdf-doc deploy (Cloudflare Pages)
rem  Double-click after changes to update the live site.
rem
rem  First-time setup (run once in this folder):
rem    npm install
rem    npx wrangler login
rem    npx wrangler pages project create sidenote-pdf-doc
rem    Create GitHub repo: https://github.com/new
rem      -> minnanosaiban/sidenote-pdf-doc (public)
rem ============================================================

echo === Deploy sidenote-pdf-doc to Cloudflare Pages ===
cd /d "%~dp0"
echo Current: %CD%
...
git add .
git commit -m "Update sidenote-pdf-doc" || echo No changes to commit
